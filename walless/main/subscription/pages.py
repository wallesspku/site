...
def _clash_sub_page(request, email, password):
    ...
    user_info = user_pool.email2user.get(email)
    ...
    if not user_info.enable:
        logger.warning(f'{email} asked for subs, but it is disabled!')
        raise Http404('User is disabled.')
    time_verify = time.time() - since
    since = time.time()

    # parse the user agent and parameters; ur contains everything that is user related.
    ur = UserRequest.from_request(request, user_info)
    sublog_cache.add((
        int(time.time()),
    ...
    response['subscription-userinfo'] = f'upload=0; download={user_info.total_data-user_info.balance}; ' \
        f'total={user_info.total_data}; expire={next_refresh()}'
